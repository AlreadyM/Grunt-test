...
module.exports = function(grunt) {
  var jshintSrc = ['Gruntfile.js','src/js/*.js'],
      uglifySrc = ['src/js/*.js','src/*.js'],uglifySave = 'build/js/<%=pkg.name%>-<%=pkg.version%>.min.js';
  var cssminSrc = '';
  ...
  grunt.initConfig({
    ...
    cssmin: {
      minify: {// 压缩css文件的配置信息
        ...
        dest: 'build/css/',// 生成的压缩文件存放路径
        ext: '.min.css'// 生成的压缩文件命名规则(.min.css)
      },
      combinebase1_base2: {// 合并css文件的配置信息 
        files: {
          'build/<%=pkg.name%>-<%=pkg.version%>.min.css': ['build/css/*.min.css']// 输出合并后的文件路径 和被合并的指定文件
    ...
    watch:{
      build:{
        files:['Gruntfile.js','src/*/*.js',"src/*/*.css"],// 此处配置被监测文件(文件夹)
        tasks:['jshint','uglify'],// 此处配置监测到变动后执行的任务（事件）
